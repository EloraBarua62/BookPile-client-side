...
const Login = () => {
  ...
  const handleForm = async (event) => {
    ...
    const url = `https://bookpile-server-side.onrender.com/login`;
    fetch(url, {
      method: "GET",
      headers: {
  ...
  //     await signInWithEmailAndPassword(email, password);
  //     const { data } = await axios.post('https://bookpile-server-side.onrender.com/login', { email });
  //     // console.log(data);
  //     localStorage.setItem('accessToken', data.accessToken);
  //     navigate(from, { replace: true });
  // }
  ...
  return (
    <div
      id="overlay"
      className="w-full flex-col items-center h-screen bg-cover bg-no-repeat bg-center bg-[url('https://img.freepik.com/free-photo/young-student-working-assignment_23-2149257248.jpg?w=740&t=st=1651814498~exp=1651815098~hmac=d49a39ef89505279e346fbbe7b8c8da17bf2dc974dcfb099c7927f601e74ca1b')]"
    >
      <div className="px-20 py-16">
        <div className="w-full bg-amber-500" style={{ height: "2px" }}></div>
        <h1 className="text-4xl text-amber-500 uppercase font-bold text-center py-1">
          login to bookpile
        </h1>
        <div className="w-full bg-amber-500" style={{ height: "2px" }}></div>
      </div>
      <div className="w-5/6 md:w-3/5 xl:w-1/2 py-20 bg-slate-900 mx-auto">
        ...
        <div className="py-5 flex">
          <div
            className="w-1/2 bg-amber-500 my-auto"
            style={{ height: "2px" }}
          ></div>
          <h1 className="text-2xl text-amber-500 uppercase font-bold text-center px-2">
            or
          </h1>
          <div
            className="w-1/2 bg-amber-500 my-auto"
        ...
        <div className="w-full flex justify-center mx-1">
          <h1 className="text-2xl font-bold text-white pr-3 my-auto uppercase">
            Log In With
          </h1>
          <GoogleAccount></GoogleAccount>
        </div>
        <div className="w-3/5 flex justify-between mx-auto mt-10 gap-x-7">
          <Link to="/signup" className="text-amber-400 text-xl">
            New to BookPile?
          </Link>
          <button
            onClick={() => handleResetPassword()}
            className="text-amber-400 text-xl ml-auto"
          >
            Forget Password?
          </button>
        </div>
